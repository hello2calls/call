...
- (void)requestPublicIPandPortWithUDPSocket:(GCDAsyncUdpSocket *)socket delegate:(id<STUNClientDelegate>)_delegate{
    ...
    //
    //
    msgTypeBindingRequest = [NSData dataWithBytes:"\x00\x01" length:2]; // STUN binding request. A Binding request has class=0b00 (request) and
    // method=0b000000000001 (Binding)
    bodyLength = [NSData dataWithBytes:"\x00\x00" length:2]; // we have/need no attributes, so message body length is zero
    magicCookie = [NSData dataWithBytes:"\x21\x12\xA4\x42" length:4]; // The magic cookie field MUST contain the fixed value 0x2112A442 in
    // network byte order.
    transactionIdBindingRequest = [self createNRundomBytes:12]; //  The transaction ID used to uniquely identify STUN transactions.
    ...
    //
    [socket sendData:stunRequest toHost:self.stunServer port:self.stunPort withTimeout:-1 tag:1002];
}
...
- (void)startSendIndicationMessage{
    ...
    //
    //
    msgTypeIndicationMessage = [NSData dataWithBytes:"\x00\x10" length:2]; // STUN indication message.
...
- (void)sendIndicationMessage{
    ...
    // network byte order.
    transactionIdIndicationMessage= [self createNRundomBytes:12]; //  The transaction ID used to uniquely identify STUN transactions.
    ...
    //
    [udpSocket sendData:stunIndicationMessage toHost:self.stunServer port:self.stunPort withTimeout:-1 tag:1003];
}
...
withFilterContext:(id)filterContext{
    ...
    if([delegate respondsToSelector:@selector(didReceivePublicIPandPort:)]){
        NSMutableDictionary *result = [NSMutableDictionary dictionaryWithObjectsAndKeys:ip, publicIPKey, port, publicPortKey, isNatPortRandom, isPortRandomization,
                                self.stunServer, publicSocketHostKey, @(self.stunPort), publicSocketPortKey, @(self.hostPort), publicHostPortKey, nil];
        if (self.bundle && [self.bundle count] > 0) {
            [result setObject:self.bundle forKey:@"bundle"];
        }
