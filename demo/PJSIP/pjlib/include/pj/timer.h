...
typedef struct pj_timer_entry
{
    ...
    /**
     * pj_timer_heap_poll will poll all timer entries, to excute expired timer jobs,
     * this timer entry poll times before expired
     */
    pj_uint8_t _poll_times;
 ...
 * @param next_timer_poll_times If this parameter is not NULL, it will be filled up with
 *           the next timer's poll count until it elapsed, or 0 if no entry exist.
 *
 * @return           The number of timers expired.
 */
PJ_DECL(unsigned) pj_timer_heap_poll( pj_timer_heap_t *ht, 
                                      pj_time_val *next_delay,
                                      pj_uint8_t *next_timer_poll_times);
