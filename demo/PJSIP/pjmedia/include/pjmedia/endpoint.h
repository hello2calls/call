...
#ifndef STREAM_FEC_RATIO_MAX_LEVELS
...
/**
 * set endpt's fec config
 * fec_enabled: 0: disable, 1:enable
 * grp_pkts : pkts per fec group (src pkts + repair pkts)
 * src_pkts : source pkts per fec group , must less than grp_pkts
 */
PJ_DECL(void) pjmedia_endpt_set_fec(pjmedia_endpt *enpt,
        int fec_enabled, int grp_pkts, int src_pkts);
...
/**
 * Get endpoint fec settings
 *
 * @param endpt		The media endpoint.
 *
 * @return void*
 */
PJ_DEF(void*) pjmedia_endpt_get_fec_ratio(pjmedia_endpt *endpt);

PJ_DEF(void) pjmedia_endpt_set_fec_ratio(pj_pool_t *pool, pjmedia_endpt *endpt,
                                         pj_str_t fec_ratio);
