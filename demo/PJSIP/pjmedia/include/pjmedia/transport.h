...
struct pjmedia_transport_op
{
    ...
    pj_status_t (*send_rtp2)(pjmedia_transport *tp,
			    const void *pkt,
			    pj_size_t size,
...
PJ_INLINE(pj_status_t) pjmedia_transport_send_rtp(pjmedia_transport *tp,
						  ...
						  pj_size_t size)
{
//    PJ_LOG(5, ("transport.h", "xxxxx pjmedia_transport_send_rtp %x %x %d", tp, pkt, size));
    return (*tp->op->send_rtp)(tp, pkt, size);
}

PJ_INLINE(pj_status_t) pjmedia_transport_send_rtp2(pjmedia_transport *tp,
						  const void *pkt,
						  pj_size_t size,
                          const pj_sockaddr * addr)
{
 /*   char ip_s[30];
    pj_sockaddr_print(addr,ip_s,30,1);
    PJ_LOG(5, ("transport.h", "xxxxx pjmedia_transport_send_rtp2 %s ,%x %x %x", ip_s, tp, pkt, addr));
    */
    pj_status_t status = (*tp->op->send_rtp2)(tp, pkt, size, addr);
    //PJ_LOG(5,("transport.h", "send_rtp2 tp=%x status= %d", tp, status));
    return status;
}
