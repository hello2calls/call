...
#include <pj/sock.h>
#include <pjmedia/rtp_auxiliary_protocol.h>
...
{
    int fec_ratio;
    int second_fec_ratio;
}fec_ratio_cfg;

typedef struct pjmedia_fec_cfg
{
    int fec_enabled;
    int grp_pkts;
    int src_pkts;
...
typedef struct pjmedia_stream_info
{
    ...
    pjmedia_fec_cfg local_fec;
    pjmedia_fec_cfg rem_fec;
} pjmedia_stream_info;
 ...
 * Set postkid addr
 */
PJ_DECL(pj_status_t) pjmedia_stream_set_postkid_addr( pjmedia_stream * stream, const pj_sockaddr* postkid_addr, int count, int test_seconds, float good_recv_rate);

/**
 * Get the last postkid addr selected
 */
PJ_DEF(pj_status_t) pjmedia_stream_get_final_postkid( pjmedia_stream * stream, pj_sockaddr * addr, unsigned * type);

PJ_DEF(pj_status_t) pjmedia_stream_set_network_type( pjmedia_stream * stream, int network_type);
/**
...
/**
 * Get output frame state. See also
 * #pjmedia_stream_get_stat_outputframe()
 *
 * @param stream	The media stream.
 * @param state		Output frame state.
 *
 * @return		PJ_SUCCESS on success.
 */
PJ_DECL(pj_status_t) pjmedia_stream_get_stat_outputframe(const pjmedia_stream *stream,
						  pjmedia_output_frame_state *state);
...
/**
 * Attach transport to stream, and detach old transport.
 */
PJ_DECL(pj_status_t)
pjmedia_stream_reattach_transport(const pjmedia_stream_info* info,
    pjmedia_transport* tp, pjmedia_stream* stream);

/**
 * Set stream keep-alive flag.
 */
PJ_DECL(void)
pjmedia_stream_set_use_ka(pjmedia_stream* stream, pj_bool_t use_ka);

/**
 * Set stream codec vad flag.
 */
PJ_DECL(void)
pjmedia_stream_set_use_vad(pjmedia_stream* stream, int vad_enabled);

/**
 * Set stream only_recv_audio flag.
 */
PJ_DECL(void)
pjmedia_stream_set_only_recv_audio(pjmedia_stream* stream, pj_bool_t only_recv_audio);

/**
 * Display stream jitter buffer state.
 */
PJ_DECL(void)
pjmedia_stream_dispaly_jb_state(pjmedia_stream* stream);

/**
 * Flush FEC buffer list if it prefetch some rtp pkts.
 *
 * @return   How many milliseconds we may wait before pause this stream.
 */
PJ_DECL(int)
pjmedia_stream_flush_fec_buffer(pjmedia_stream* stream);
...
int pjmedia_stream_get_rtp_jbuf_total(pjmedia_stream *stream);

int pjmedia_stream_get_rtp_jbuf_loss(pjmedia_stream *stream);

PJ_DEF(pj_status_t) pjmedia_stream_set_decode_grp_stat(pjmedia_stream *stream);

PJ_DEF(pj_status_t) start_relay_encode(rtp_start_relay *data, pj_bool_t is_type_postman);

PJ_DEF(pj_status_t) stop_relay_encode(rtp_stop_relay *data);
