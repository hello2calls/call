...
/**
 * Convert CoreAudio error code to pjmedia error code.
...
/**
 * Convert pjmedia error space to CoreAudio error codes.
 */
#define PJMEDIA_AUDIODEV_ERRNO_TO_COREAUDIO(err,dest_err) \
 do {\
 	if (err >= PJMEDIA_AUDIODEV_OPENSLES_ERROR_START\
 		&& err <= PJMEDIA_AUDIODEV_COREAUDIO_ERRNO_START) {\
 		dest_err = ((int)PJMEDIA_AUDIODEV_COREAUDIO_ERRNO_START-err);\
 	} else {\
 		dest_err = err;\
 	}\
 } while (0)
...
/**
 * Mapping from OpenslES error code to pjmedia error space.
 */
#define PJMEDIA_AUDIODEV_OPENSLES_ERROR_START \
 		(PJMEDIA_AUDIODEV_BDIMAD_ERROR_END + 1)
#define PJMEDIA_AUDIODEV_OPENSLES_ERROR_END	\
 		(PJMEDIA_AUDIODEV_OPENSLES_ERROR_START + 1000 - 1)
/**
 * Convert OpenslES error codes to pjmedia error space.
 * OpenslES error code range: 0 <= err <= 16
 */
#define PJMEDIA_AUDIODEV_ERRNO_FROM_OPENSLES(err) \
		((int)PJMEDIA_AUDIODEV_OPENSLES_ERROR_START+err)
/**
 * Convert pjmedia error space to OpenslES error codes.
 */
#define PJMEDIA_AUDIODEV_ERRNO_TO_OPENSLES(err,dest_err) \
 do {\
 	if (err >= PJMEDIA_AUDIODEV_OPENSLES_ERROR_START\
 		&& err <= PJMEDIA_AUDIODEV_OPENSLES_ERROR_END) {\
 		dest_err = (err - (int)PJMEDIA_AUDIODEV_OPENSLES_ERROR_START);\
 	} else {\
 		dest_err = err;\
 	}\
 } while (0)
...
/**
 * @hideinitializer
 * Recorder device open fail
 */
#define PJMEDIA_EAUD_REC_DEV_FAIL (PJMEDIA_AUDIODEV_ERRNO_START+13) /* 4200013 */
