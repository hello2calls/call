...
typedef struct pjsua_callback
{
    ...
    /**
     * Notify application about the response status of outgoing OPTIONS
     * request.
     *
     * @param call_id	    Containts the ID of the call where the query was
     *			    sent, or PJSUA_INVALID_ID if the query was sent
     *			    outside call context.
     * @param to	    Destination URI.
     * @param user_data	    Arbitrary data that was specified when sending
     *			    OPTIONS request.
     * @param status	    SIP status code.
     * @param reason	    SIP reason.
     * @param tdata	    The original OPTIONS request.
     * @param rdata	    The incoming OPTIONS response, or NULL if the
     *			    message transaction fails because of time out
     *			    or transport error.
     * @param acc_id	    Account ID from this the original request was
     *			    sent.
     */
    void (*on_options_response)(pjsua_call_id call_id,
			        const pj_str_t *to,
			        void *user_data,
			        pjsip_status_code status,
			        const pj_str_t *reason,
    ...
    /**
     * Notify application about the response status of INFO
     * request.
     * @param to	    Destination URI.
     * @param user_data	    Arbitrary data that was specified when sending
     *			    OPTIONS request.
     ...
     *			    sent.
     */
    void (*on_info_response)(const pj_str_t *to,
                                void *user_data,
                                pjsip_status_code status,
                                const pj_str_t *reason,
                                pjsip_tx_data *tdata,
...
/**
 * Modify account header information.
 *
 * @param acc_id	Id of the account to be modified.
 * @param hdr_list	New account header.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_acc_set_register_headers(pjsua_acc_id acc_id,
 ...
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_acc_keep_alive(pjsua_acc_id acc_id);

/**
 * disconnect TCP connect channel.
 *
 * @param acc_id	Account identification.
 *
 * @return		PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_acc_tcp_disconnect(pjsua_acc_id acc_id);
...
typedef struct pjsua_stream_stat
{
    ...
    /** frame stat statistic. */
    pjmedia_output_frame_state  framestat;
...
/*
 * Set postkid address for audio stream
 */
PJ_DECL(pj_status_t) pjsua_call_stream_set_postkid_addr( pjsua_call_id call_id,
                                                        unsigned med_idx,
                                                        const char* postkid_addr,
                                                        unsigned addr_len,
                                                        int test_seconds,
                                                        float good_recv_rate);
/*
 * Set network type for audio stream
 */
PJ_DECL(pj_status_t) pjsua_call_stream_set_network_type( pjsua_call_id call_id,
                                                        unsigned med_idx,
                                                        int network_type);
/*
 * Get the final postkid. Returns empty string if no postkid is selected
 */
PJ_DECL(pj_status_t) pjsua_call_get_final_postkid_addr( pjsua_call_id call_id,
                                                        unsigned med_idx,
                                                        char * buf);
/**
...
/**
 * Send OPTIONS request at any time.
 *
 * @param acc_id        Account ID to be used to send the request.
 * @param to            Remote URI.
 * @param msg_data      Optional list of headers etc to be added to outgoing
 *                      request.
 * @param user_data     Optional user data, which will be given back when
 *                      the OPTIONS callback is called.
 * @return              PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_options_send(pjsua_acc_id acc_id,
                                        const pj_str_t *to,
                                        const pjsua_msg_data *msg_data,
                                        void *user_data);

/**
 * Send INFO request at any time.
 *
 * @param acc_id        Account ID to be used to send the request.
 * @param to            Remote URI.
 * @param msg_data      Optional list of headers etc to be added to outgoing
 *                      request.
 * @param user_data     INFO user data, which will be given back when
 *                      the INFO callback is called.
 * @return              PJ_SUCCESS on success, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsua_info_send(pjsua_acc_id acc_id,
                                     const pj_str_t *to,
                                     const pjsua_msg_data *msg_data,
                                     void *user_data);
...
struct pjsua_media_config
{
     ...
     * 0-off, 1-conservative, 2-moderate, 3-aggresive
     */
    pj_int32_t aec_option;

    pj_int32_t aec_latency;

    /**
     * noise suppression options:
     * 0-off, 1-conservative, 2-moderate, 3-aggresive
     */
    pj_int32_t ns_option;

    /**
     * speaker agc_threshold (negative db, max is 0db)
     */
    int agc_threshold;

    /**
     * speaker agc_gain (db, positive)
     */
    int agc_gain;
