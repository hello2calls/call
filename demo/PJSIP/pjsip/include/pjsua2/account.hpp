...
struct OnIncomingCallParam
{
    ...
    SipRxData		rdata;
    /**
     * The incoming INVITE msg.
     */
    string          incomingMsg;
    /**
     * The incoming INVITE msg length.
     */
    int             incomingMsgLen;
};
...
/**
 * Parameters for onOptionsResponse() account callback.
 */
struct OnOptionsResponseParam
{
    /**
     * Token or a user data that was associated with the OPTIONS
     * request.
     */
    Token		userData;
...
/**
 * Parameters for onInfoResponse() account callback.
 */
struct OnInfoResponseParam
{
    /**
     * Token or a user data that was associated with the OPTIONS
     * request.
...
public:
    ...
    /**
     * Update the cuurent account header value.This won't call reregistration,
     * but when setRegistration called ,the update will take effect.
     * @param new header list to be applied to current account
     * @return
     */
    void setRegisterHeaders(const SipHeaderVector &reg_hdr_list) throw(Error);
     ...
     *
     */
    void doKeepAlive() throw(Error);
    
    /**
     * disconnect Tcp connection
     *
     */
    void disconnectTcpChannel() throw(Error);
    ...
    void sendOptionsRequest(const SendOptionsRequestParam &prm) throw(Error);

    void sendInfoRequest(const SendInfoRequestParam &prm) throw(Error);
public:
    /*
     * Callbacks
     */
    ...
    /**
     * Notify application about OPTIONS response.
     *
     * @param prm	    Callback parameter.
     */
    virtual void onOptionsResponse(OnOptionsResponseParam &prm)
    { PJ_UNUSED_ARG(prm); }
    
    /**
     * Notify application about INFO response.
     *
     * @param prm	    Callback parameter.
     */
    virtual void onInfoResponse(OnInfoResponseParam &prm)
    { PJ_UNUSED_ARG(prm); }
...
private:
    ...
    pj_str_t             tmpRegistrarUri;   // for use in sendOptionsRequest()
};
