...
public:
    ...
    int libHandleEventsWithoutPendingJobs(unsigned msec_timeout);
    ...
    void transportAddRef(TransportHandle hnd) throw(Error);

    void transportDecRef(TransportHandle hnd) throw(Error);

    void transportSendKa(TransportHandle hnd) throw(Error);
    ...
    /*************************************************************************
     * Global Sip config operations
     */
    
    /**
     * Set the interval for periodic keep-alive transmission.
     *
     * @param ka_interval interval in seconds.
     */
    void setTcpKaInterval(unsigned ka_interval);

    /**
     * Specify the data to be transmitted as keep-alive packets.
     *
     * @param ka_data keep-alive body for periodic transmission.
     */
    void setTcpKaData(const string &ka_data);
...
public:
    ...
    // be invoked when timer heap schedule a new timer entry
    virtual void onTimerSchedule(int msec, int poll_times)
    { PJ_UNUSED_ARG(msec); }
...
private:
    ...
    static void on_timer_schedule_callback(pjsip_endpoint *endpt,
        const pj_time_val *delay, const pj_uint8_t *poll_times);
...
private:
    ...
    static void on_options_response(pjsua_call_id call_id,
                                    const pj_str_t *to,
                                    void *user_data,
                                    pjsip_status_code status,
                                    const pj_str_t *reason,
    ...
    static void on_info_response(const pj_str_t *to,
                                 void *user_data,
                                 pjsip_status_code status,
                                 const pj_str_t *reason,
                                 pjsip_tx_data *tdata,
