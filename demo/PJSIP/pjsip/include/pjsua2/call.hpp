...
struct RtcpStat
{
    ...
    pj_uint32_t rtp_jbuf_total;

    pj_uint32_t rtp_jbuf_loss;

    pj_uint32_t normal_frame_cnt;

    pj_uint32_t missing_frame_cnt;

    pj_uint32_t zero_empty_frame_cnt;

    pj_uint32_t zero_prefetch_cnt;

    pj_uint32_t plc_cnt;

     pj_uint32_t repaired_cnt;

     pj_uint32_t loss_cnt;

     pj_uint32_t total_cnt;

     pj_uint32_t rtp_put_src_pkt_cnt;       /* rtp put src pkt cnt              */

     pj_uint32_t rtp_put_fake_pkt_cnt;

     pj_uint32_t rtp_low_recv_rate_cnt;     /* rtp low recv rate cnt */

     pj_uint32_t rtp_put_decoder_cnt;       /* rtp pkt put in decoder cnt */

     pj_uint32_t rtp_group_cnt;                 /* rtp group cnt */

     pj_uint32_t rtp_invalid_group_cnt;           /* rtp valid group cnt */

     pj_uint32_t rtp_lost_cnt;         /* rtp valid src cnt */

     pj_uint32_t rtp_list_full_cnt;

    //string rtp_group_pkt_miss_cnt;         /* rtp group miss pkt cnt */
     //pj_uint32_t rtp_start_seq;

     //pj_uint32_t rtp_end_seq;

     pj_uint32_t rtp_list_empty_cnt;             /* rtp list empty cnt */

     pj_uint32_t rtp_list_burst_cnt;        /* jb burst cnt, if put frame in jb */

     pj_uint32_t rtp_list_prefetch_cnt;     /* rtp list prefetch cnt */

     pj_uint32_t rtp_too_late_cnt;          /* rtp too late cnt */

     pj_uint32_t rtp_seq_jump_cnt;              /* rtp jump cnt */

     pj_uint32_t rtp_grp_reverse_cnt;                          /* rtp seq reverse cnt              */

     pj_uint32_t rtp_grp_gap2_cnt;                             /* rtp max gap cnt                  */

     pj_uint32_t rtp_grp_gap3_cnt;                             /* rtp max gap cnt                  */

     pj_uint32_t rtp_grp_gapbig_cnt;                           /* rtp max gap cnt                  */

     pj_uint32_t rtp_grp_gap_max;                              /* rtp gap max cnt                  */

     pj_uint32_t rtp_grp_blank_fec_cnt;                        /* rtp grp blank fec cnt            */

     pj_uint32_t rtp_grp_del_cnt;

     pj_uint32_t rtp_pkt_seq_err_cnt;            /* rtp pkt seq err cnt */

     pj_uint32_t rtp_consecutive_empty_cnt;      /* rtp consecutive_empty_cnt */

     pj_uint32_t rtp_empty_frm_max;              /* rtp max empty frm case */

     pj_uint32_t rtp_resend_cnt;

     pj_uint32_t repaired_cnt_2;

     pj_uint32_t rtp_group_pkt_miss1_cnt;

     pj_uint32_t rtp_group_pkt_miss2_cnt;

     pj_uint32_t rtp_group_pkt_miss3_cnt;

     pj_uint32_t rtp_group_pkt_miss4_cnt;

     pj_uint32_t rtp_group_pkt_miss5_cnt;

     pj_uint32_t postkid_cur_state;

     pj_uint32_t postkid_single2double_cnt;

     pj_uint32_t postkid_double2single_cnt;
...
/**
 * Media stream output frame statistic.
 */
//struct StreamFrameStat
//{
    /* Statistic */
    /*
    unsigned int normal_frame_cnt;

    unsigned int missing_frame_cnt;

    unsigned int zero_empty_frame_cnt;

    unsigned int zero_prefetch_cnt;

    unsigned int plc_cnt;

    unsigned int repaired_cnt;

    unsigned int loss_cnt;

    unsigned int total_cnt;

    unsigned int rtp_put_src_pkt_cnt;       /* rtp put src pkt cnt              */

    //unsigned int rtp_low_recv_rate_cnt;     /* rtp low recv rate cnt */

    //unsigned int rtp_put_decoder_cnt;       /* rtp pkt put in decoder cnt */

    //unsigned int rtp_group_cnt;                 /* rtp group cnt */

    //unsigned int rtp_invalid_group_cnt;           /* rtp valid group cnt */

    //string rtp_group_pkt_miss_cnt;         /* rtp group miss pkt cnt */

    //unsigned int rtp_list_empty_cnt;             /* rtp list empty cnt */

    //unsigned int rtp_list_burst_cnt;        /* jb burst cnt, if put frame in jb */

    //unsigned int rtp_list_prefetch_cnt;     /* rtp list prefetch cnt */

    //unsigned int rtp_too_late_cnt;          /* rtp too late cnt */

    //unsigned int rtp_seq_jump_cnt;              /* rtp jump cnt */

    //unsigned int rtp_pkt_seq_err_cnt;            /* rtp pkt seq err cnt */

    //unsigned int rtp_consecutive_empty_cnt;      /* rtp consecutive_empty_cnt */

    //unsigned int rtp_empty_frm_max;              /* rtp max empty frm case */

    //unsigned int repaired_cnt_2;

    //unsigned int rtp_group_pkt_miss1_cnt;

    //unsigned int rtp_group_pkt_miss2_cnt;

    //unsigned int rtp_group_pkt_miss3_cnt;

    //unsigned int rtp_group_pkt_miss4_cnt;

//public:
    /**
     * Convert from pjsip
     */
    //void fromPj(const pjmedia_output_frame_state &prm);
...
public:
    ...
    /**
     * Set postkid addr to audio stream
     *
     * @param med_idx       Media stream index.
     *
     * @param postkid_addr  postkid address
     */
    void setStreamPostkidAddr(unsigned med_idx, SocketAddress postkid_addr, int test_seconds, float good_recv_rate);
    /**
     * Set network type
     *
     * @param med_idx       Media stream index.
     *
     * @param network_type 1 wifi 0 other
     */
    void setStreamNetworkType(unsigned med_idx, int network_type);
    /**
     * Get final postkid addr selected
     *
     */
    string getFinalPostkidAddr(unsigned med_idx);
...
public:
    ...
    void setOpParamOfDel(CallOpParam param) { opParamOfDel = param; }
