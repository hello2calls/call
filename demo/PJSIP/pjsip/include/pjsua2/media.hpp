...
public:
    ...
    // set playback and capture device in same function, because playback and
    // capture device must belong same factory.
    // so set them separately will cause assert failed
    void setPlayBackAndCaptureDev(int playback_dev, int capture_dev) const throw(Error);
