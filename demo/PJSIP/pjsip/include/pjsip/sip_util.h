...
/**
 * Generate Call-ID value with from/to header and timestamp in second.
 *
 * @param from: from header of msg
 * @param to: to header of msg
 * @param str: output unique call-id
 * @return PJ_SUCCESS, or the appropriate error code.
 */
PJ_DECL(pj_status_t) pjsip_generate_call_id(pj_pool_t *pool,
    const pjsip_from_hdr *from, const pjsip_to_hdr *to, pj_str_t *str);
