...
/**
 * The type of callback function to be called when endpt scheduled timer
 *
 * @param delay          The interval of next timer to expire.
 * @param poll_times     The poll times before next timer expired.
 */
typedef void pjsip_endpt_schedule_callback(pjsip_endpoint *endpt,
				    const pj_time_val *delay, const pj_uint8_t *poll_times);


PJ_DECL(void) pjsip_endpt_set_timer_schedule_callback(pjsip_endpoint *endpt,
        pjsip_endpt_schedule_callback *cb);

/**
 * wakeup from ioqueue poll
 */
PJ_DECL(void) pjsip_endpt_wakeup( pjsip_endpoint *endpt );
